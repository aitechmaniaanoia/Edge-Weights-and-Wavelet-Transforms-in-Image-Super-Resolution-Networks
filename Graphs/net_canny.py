...
class CannyNet(nn.Module):
    def __init__(self, threshold=0.5, use_cuda=False):
    ...
    def forward(self, img):
        ...
        value_r =grad_x_r**2 + grad_y_r**2
        value_g =grad_x_r**2 + grad_y_r**2
        value_b =grad_x_r**2 + grad_y_r**2
        grad_mag = torch.sqrt(value_r)+\
                   torch.sqrt(value_g)+\
                   torch.sqrt(value_b)  
        ...
        batch = inidices_positive.shape[0]
        ...
        max_values = torch.max(thresholded.view(batch,-1),dim=1,keepdim=True).values
        max_values = torch.unsqueeze(torch.unsqueeze(max_values,dim=2),dim=3)
        max_values[max_values==0] = 1.0

        if self.use_cuda:
            max_values = max_values.cuda()

        thresholded = thresholded / max_values
        thresholded[thresholded<self.threshold] = 0.0
        # thresholded[thresholded>0] = 1.0
