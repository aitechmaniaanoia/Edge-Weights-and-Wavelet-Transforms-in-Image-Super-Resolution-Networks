...
class SRSolverWL(BaseSolver):
    def __init__(self, opt):
        super(SRSolverWL, self).__init__(opt)
        self.train_opt = opt['solver']
        self.LR = self.Tensor()
        self.HR = self.Tensor()
        self.HR_edge = self.Tensor()
        self.HR_dwt = self.Tensor()
        self.SR = None
        self.wl_lamda = self.opt['solver']['wl_lamda']
        ...
        if self.is_train:
            ...
            if loss_type == 'l1':
                ...
                self.criterion_pix_edge = nn.L1Loss()
                self.criterion_pix_dwt = nn.L1Loss()
            elif loss_type == 'l2':
                self.criterion_pix = nn.MSELoss()
                self.criterion_pix_edge = nn.MSELoss()
                self.criterion_pix_dwt = nn.MSELoss()
            else:
                raise NotImplementedError('Loss type [%s] is not implemented!'%loss_type)

            if self.use_gpu:
                self.criterion_pix = self.criterion_pix.cuda()
                self.criterion_pix_edge = self.criterion_pix_edge.cuda()
                self.criterion_pix_dwt = self.criterion_pix_dwt.cuda()
    ...
    def feed_data(self, batch, need_HR=True):
        ...
        hr_edge = batch['HR_edge']
        self.HR_edge.resize_(hr_edge.size()).copy_(hr_edge)

        hr_dwt = batch['HR_dwt']
        self.HR_dwt.resize_(hr_dwt.size()).copy_(hr_dwt)
    ...
    def train_step(self):
        ...
        for i in range(self.split_batch):
            ...
            split_HR_edge = self.HR_edge.narrow(0, i*sub_batch_size, sub_batch_size)
            split_HR_dwt = self.HR_dwt.narrow(0, i*sub_batch_size, sub_batch_size)

            if self.use_cl:
                outputs,edges,wvs = self.model(split_LR)
                loss_steps = [self.criterion_pix(sr, split_HR) for sr in outputs]
                loss_edge_steps = [self.criterion_pix_edge(sr_edge, split_HR_edge) for sr_edge in edges]
                loss_wv_steps = [self.criterion_pix_dwt(sr_wv, split_HR_dwt) for sr_wv in wvs]

                for step in range(len(loss_steps)):
                    loss_value = self.wl_lamda[0] * loss_steps[step] + self.wl_lamda[1] * loss_wv_steps[step] + self.wl_lamda[2] * loss_edge_steps[step] 
                    loss_sbatch += self.cl_weights[step] * loss_value
            else:
                output,edges,wvs = self.model(split_LR)
                loss_sbatch = self.wl_lamda[0] * self.criterion_pix(output, split_HR)+\
                              self.wl_lamda[1] * self.criterion_pix_dwt(wvs, split_HR_dwt)+\
                              self.wl_lamda[2] * self.criterion_pix_edge(edges, split_HR_edge)
    ...
    def _overlap_crop_forward(self, x, shave=10, min_size=100000, bic=None):
        ...
        if w_size * h_size < min_size:
            ...
            for i in range(0, 4, n_GPUs):
                ...
                sr_batch_temp,_,_ = self.model(lr_batch)
    ...
    def get_current_visual_wv(self, need_np=True, need_HR=True):
        """
        return LR SR (HR) images
        """
        out_dict = OrderedDict()
        ...
        lr_wavelet = self.LR.data[0].float().cpu()
        sr_wavelet = self.SR.data[0].float().cpu()

        out_dict['LR'] = self.get_img_from_wv(lr_wavelet, wavelet_name)
        out_dict['SR'] = self.get_img_from_wv(sr_wavelet, wavelet_name)

        if need_np:  out_dict['LR'], out_dict['SR'] = util.Tensor2np([out_dict['LR'], out_dict['SR']],
                                                                        self.opt['rgb_range'])
        if need_HR:
            out_dict['HR'] = self.get_img_from_wv(self.HR.data[0].float().cpu(), wavelet_name)
            if need_np: out_dict['HR'] = util.Tensor2np([out_dict['HR']],
                                                           self.opt['rgb_range'])[0]
        return out_dict

    def get_img_from_wv(self, wavelet, wavelet_name):
        lr_r = wavelet[0:4,:,:].numpy()
        lr_g = wavelet[4:8,:,:].numpy()
        lr_b = wavelet[8:12,:,:].numpy()
        # lr = wavelet.numpy()
        # lr = lr.transpose((1,2,0))

        # coeff = [lr[:,:,0:3],({'ad':lr[:,:,3:6],'da':lr[:,:,6:9],'dd':lr[:,:,9:12]})]

        coeff_r = [(lr_r[0,:,:],(lr_r[1,:,:],lr_r[2,:,:],lr_r[3,:,:]))]
        coeff_g = [(lr_g[0,:,:],(lr_g[1,:,:],lr_g[2,:,:],lr_g[3,:,:]))]
        coeff_b = [(lr_b[0,:,:],(lr_b[1,:,:],lr_b[2,:,:],lr_b[3,:,:]))]

        img_r = torch.from_numpy(pywt.iswt2(coeff_r, wavelet=wavelet_name, norm=True))
        img_g = torch.from_numpy(pywt.iswt2(coeff_g, wavelet=wavelet_name, norm=True))
        img_b = torch.from_numpy(pywt.iswt2(coeff_b, wavelet=wavelet_name, norm=True))
        # img = torch.from_numpy(pywt.iswtn(coeff,wavelet=wavelet_name,norm=True,axes=(0,1)))

        img = torch.stack([img_r,img_g,img_b], dim=0)

        return img

    def save_current_visual(self, epoch, iter, use_wavelet):
        """
        ...
        if epoch % self.save_vis_step == 0:
            visuals_list = []
            if use_wavelet:
                visuals = self.get_current_visual_wv(need_np=False)
            else:
            ...
            cv2.imwrite(os.path.join(self.visual_dir, 'epoch_%d_img_%d.png' % (epoch, iter + 1)),
                        visual_images)
